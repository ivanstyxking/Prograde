      ...
      if (mousePressed) {
        if (rightgun) {
          if (cooldown2<=0) {
            ...
            bullets.add(new Bullet(velocity.mag()+250+random(-4, 4), 0, 0, 20, difference.heading()+randomGaussian()*PI/128, ship.position.x+ship.velocity.x*2, ship.position.y+ship.velocity.y*2, false, 2));
            cooldown2=23;
